...
def main(argv=None):
    ...
    try:
        ...
        if not os.path.isfile(configpath):
            print u"Bienvenido/a al asistente personalizado de configuración de Vortexz(Version XeniBot by Xenial). Copyleft 2018 Freuddy Hernandez. http://Vortexz.blogspot.com\n"
        ...
        if os.path.isfile(pid_file_path):
            ...
            if tools.check_pid(old_pid):
                if opts.quit is None and opts.kill is None:
                    stderr(u'There is already a Vortexz running. -- Ya hay un bot ejecutándose. -- Il y a déjà un bot en fonctionnement.')
                    stderr(u'Try -- Intenta: --quit o --kill')
                    sys.exit(1)
                elif opts.kill:
                    stderr(u'Killing Vortexz. -- Matando a Vortexz. -- Tuent Vortexz.')
                    os.kill(old_pid, signal.SIGKILL)
                    sys.exit(0)
                elif opts.quit:
                    stderr(u'Quitting Vortexz. -- Desconectando a Vortexz. -- Déconnectant Vortexz.')
