...
class Config(object):
    def __init__(self, filename, load=True, ignore_errors=False):
        ...
        if load:
            ...
            if not self.parser.has_option('core', 'user'):
                self.parser.set('core', 'user', 'Vortexz')
    ...
    def _core(self):
        ...
        if lang == 'en':
            ...
            print(u"\nHas configurado un idioma para el asistente personalizado de configuración de Vortexz, pero el idioma del bot en IRC puede " +
            ...
            self.interactive_add('core', 'lang', 'Idioma por defecto',
                            'es')
        elif lang == 'fr':
            ...
            sys.exit()
        if lang == 'en':
            print("\nFirst of all, you need to set up a nickname for your bot. " +
            #u"will connect to a network with the official version of Vortexz, your bot will not be able to connect. " +
            ...
            self.interactive_add('core', 'port', 'Enter the port to connect on',
                                 default_port)
            print("\nVortexz has some commands that can only be performed by its owner (probably you). " +
            ...
            print(u"\nIntroduce el nick que desees usar en el bot. Por defecto es Vortexz, " +
            u"pero puedes usar uno que sea de tu propiedad o aún no esté registrado en NickServ. " +
            u"Puedes verificar el estado del nick con un /msg NickServ info <nick> en IRC. " +
            u"Si el nick no está aún registrado, ¡no olvides hacerlo!.\n")
            self.interactive_add('core', 'nick', 'Nick del bot',
                                 'Vortexz')
            print(u"\nEvidentemente, el bot es inútil si no se conecta en una red IRC. Escoje un servidor y busca " +
            u"su 'hostname'. Si no sabes su hostname, contacta con los administradores de la red.\n")
            self.interactive_add('core', 'host', 'Servidor donde el bot debe conectarse',
                                 'irc.freenode.net')
            ...
            self.add_option('core', 'use_ssl', '¿El bot debe conectarse usando una conexión SSL?')
            if self.use_ssl == 'True':
                default_port = '6697'
            else:
                default_port = '6667'
            print(u"\nAdemás del servidor tienes que escoger el puerto. El puerto por defecto es 6667 para conexiones " +
            u"sin SSL. Si has escogido conectarte con SSL, el puerto por defecto es 6667.\n")
            self.interactive_add('core', 'port', 'Puerto del servidor que el bot usará',
                                 default_port)
            ...
            c = (u"\nEn este proceso el asistende personalizado de configuración te preguntará el nombre del  canal o canales en donde el bot entrará de manera automática. " +
            u" Para ello introduce el nombre del canal (incluida/s la/s almohadilla/s que contenga" +
            u"Puedes añadir más de un canal pulsando la tecla Intro." +
                ' Cuando hayas insertado todos los canales, vuelve a presionar Intro.')
            self.add_list('core', 'channels', c, 'Canal:')
        elif lang == 'fr':
            print(u"\nPour débuter, t'as besoin de configurer un pseudo pour ton bot. Peut-être 'Vortexz', mais si tu veux que " +
            u"Vortexz se connecte à un réseau IRC où il y a déjà un Vortexz officiel, le bot ne pourra pas " +
    ...
    if not os.path.isdir(dotdir):
        if create:
            ...
            try:
                ...
                print >> sys.stderr, \
                    'Please fix this and then run Vortexz again. -- Por favor, arregla eso y vuelve a ejecutar a Vortexz -- S\'il te plait, corriger et relancer Vortexz.'
                sys.exit(1)
        else:
...
def create_config(configpath):
    check_dir()
    global lang
    lang = raw_input('Selecciona uno de los tres idiomas disponibles: (en, es, fr) [Idioma por defecto: ES] ')
    if lang == "":
        lang = "es"
    if lang == 'en':
    ...
    try:
        ...
        config._core()
        if config.option("¿Te gustaría configurar una base de datos ahora?"):
            config._db()
        if config.option(
            '¿Te gustaría comprobar si algún módulo del bot'
