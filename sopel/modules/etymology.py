...
def etymology(word):
    ...
    if len(word) > 25:
        if bot.config.lang == 'fr':
            raise ValueError("Mot trop longue: %s[...]" % word[:10])
...
@commands('ety', 'etymology', 'etimologia', 'etimologie')
def f_etymology(bot, trigger):
    ...
    try:
        ...
        if bot.config.lang == 'fr':
            msg = "Je ne peux pas me connecter à etymonline.com (%s)" % (etyuri % word)
        elif bot.config.lang == 'es':
            msg = "No puedo conectarme a etymonline.com (%s)" % (etyuri % word)
        else:
    ...
    if result is not None:
        ...
        uri = etysearch % word
        if bot.config.lang == 'fr':
            msg = """Je n'ai trouvé rien pour "%s". Tenter l'enllaç: %s'""" % (word, uri)
        elif bot.config.lang == 'es':
