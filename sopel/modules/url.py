...
def configure(config):
    ...
    if config.option('Exclude certain URLs from automatic title display', False):
        ...
        if config.lang == 'fr':
            config.add_list('url', 'exclude', u'Expression régulier pour chaque URL que tu veux exclure.',
                'Regex:')
            config.interactive_add('url', 'exclusion_char',
                u'Prefix pour éviter que le bot cherche information des liens', '!')
        elif config.lang == 'es':
            ...
            config.add_list('url', 'exclude', 'Enter a regex for each URL you would like to exclude.',
                'Regex:')
            config.interactive_add('url', 'exclusion_char',
