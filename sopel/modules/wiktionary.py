...
def mw_snippet(server, query, bot):
    ...
    if bot.config.lang == 'fr':
        snippet_url = ('https://fr.wiktionary.org/w/api.php?format=json'
                '&action=query&prop=extracts&exintro&explaintext'
                '&exchars=300&redirects&titles=')
    elif bot.config.lang == 'es':
        snippet_url = ('https://es.wiktionary.org/w/api.php?format=json'
                '&action=query&prop=extracts&exintro&explaintext'
                '&exchars=300&redirects&titles=')
    else:
        snippet_url = ('https://en.wiktionary.org/w/api.php?format=json'
                '&action=query&prop=extracts&exintro&explaintext'
                '&exchars=300&redirects&titles=')
    if bot.config.lang == 'fr' or bot.config.lang == 'es':
...
@commands('wiktionary', 'wikcionario', 'wikt', 'wiktionnaire')
def wiktionary(bot, trigger):
    ...
    if not query:
        ...
        return NOLIMIT
    if bot.config.lang == 'fr':
        server = 'fr.wiktionary.org'
    elif bot.config.lang == 'es':
        server = 'es.wiktionary.org'
    else:
    ...
    if bot.config.lang == 'fr':
        bot.say('"%s" - http://fr.wiktionary.org/wiki/%s' % (snippet, query))
    elif bot.config.lang == 'es':
        bot.say('"%s" - http://es.wiktionary.org/wiki/%s' % (snippet, query))
    else:
        bot.say('"%s" - http://en.wiktionary.org/wiki/%s' % (snippet, query))

@rule(r".*\[\[.+\]\]")
def show_wikilink(bot, trigger):
    page = trigger.group(0).split('[[')[1].split(']]')[0]
    if not bot.config.has_option("core", "wiki_link"):
        link = 'https://%s.wiktionary.org/wiki/' % bot.config.lang
    else:
