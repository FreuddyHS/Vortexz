...
class MockSopel(object):
    def __init__(self, nick, admin=False, owner=False):
        ...
        self.config = sopel.config.Config('', load=False)
        self._init_config()
...
def get_example_test(tested_func, msg, results, privmsg, admin,
    ...
    def test():
        ...
        origin = sopel.irc.Origin(bot, hostmask, origin_args, tags)
        trigger = sopel.bot.Sopel.Trigger(
        ...
        for _i in xrange(repeat):
            wrapper = MockSopelWrapper(bot, origin)
